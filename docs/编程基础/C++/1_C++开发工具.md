...
!!!note
    Linux下直接安装gnu-make，运行命令是```make```，Windows下可以安装gcc（配套有mingw32-make），不过还是推荐使用wsl

对于大型项目的编译，有成百上千个工程文件，根本不可能纯手工去指定如何编译。又比如，你修改了一个文件的某一行代码，难道你必须要把整个项目又编译一次吗？这种时候Makefile就显得非常有用,makefile可以大写，也可以小写，一般Makefile首字母使用大写。
...
C++不像后来的Python和Java有那种很公认的包管理器，但是终归还是有替代方案的，比如vcpkg，这个可以和 VS 适配，在配置一些比如OpenGL,OpenCV的库的时候很好用，与VS建立连接之后甚至可以自动导入环境，在实际使用时引用头文件就可以了
