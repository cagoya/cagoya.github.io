...
![](images/ch4/UDP_pseudo_header.png)
...
![](images/ch4/three_way_handshake.png){width="300"}
...
![](images/ch4/four_way_handshake.png){width="300"}
...
![](images/ch4/flow_control.png){width="500"}

TCP 流量控制的过程如上图所示，接收方一共有400B缓存：

1. 发送方发了 2KB，接收方返回的窗口大小(rwnd)为2028，表示最大再传2KB
2. 发送方又发送了2KB，接收方缓存满了，返回的窗口大小为0
3. 发送方停止发送，启动持续计时器，若计时器超时，则发送一个零窗口探测报文段，接收方收到后给出现在窗口大小。
   1. 若窗口值大于0，则可以继续发送
   2. 若窗口值仍等于0，则重置持续计时器
...
![](images/ch4/congestion_control.png){width="500"}
