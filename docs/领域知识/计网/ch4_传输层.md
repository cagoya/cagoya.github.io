...
!!!note 为什么感觉计网越往上越简单？
    实际上并非因为上层协议本身简单，而是考试内容相对简单。此外，计算机专业的学生通常对传输层及以上的协议更加熟悉。传输层在网络编程中应用广泛，因此其接口设计需要保持简洁易用。
...
!!!question 之前说过网络层可以分为面向连接和无连接的实现，为什么传输层还要提供这两种服务呢？
    因为网络层的实现取决于ISP，事实上平时我们用的网络层服务都是无连接的。而传输层跑在用户机上，可以由用户选择是否有连接。

### 寻址

端口使得应用层的各种进程能够将其数据通过端口向下交付给传输层，同时也让传输层知道应当将其报文段中的数据向上通过端口交付给应用层的相应进程。端口的作用是标识主机上的进程。这里的端口是软件概念（虽然"端口"这个词最初来自硬件），并且是一个本地概念。

端口号长度为16bit，能够表示65536个不同的端口号。显然，没有一台机器能够真正用完这些端口，这也是网络层能够使用NAT的原因之一。端口号可以分为以下几类：

- **服务器端使用的端口号**：
  - **熟知端口号（well-known）**：数值为0~1023，这些端口号被分配给了TCP/IP中最重要的一些应用程序，例如HTTP使用80端口，HTTPS使用443端口
...
UDP 仅在IP层的数据报之上增加了复用、分用和差错检测功能。若应用开发者选择UDP，则应用程序几乎是直接和IP打交道。TCP并不完全好过UDP，UDP还是有自己的优点：

1. 无连接：不会引入建立连接的时延
2. 无连接状态：UDP不维护连接状态，也不跟踪连接参数
3. 首部开销小：TCP有20B的首部开销，UDP仅有8B开销
4. UDP没有拥塞控制：网络中拥塞不会影响源主机的发送速率
5. UDP支持一对一、一对多、多对一和多对多的交互通信

!!!tip 实际真的会使用UDP吗？
    大部分情况下不会直接使用裸的UDP，因为哪怕可以容忍丢包，乱序一般都是不可接受的。实际要么会在应用层添加可靠机制，要么会改进UDP
...
接收方的传输层在收到UDP报文后，会查看目的端口将其上交给应用层对应程序，但是如果发现不存在对应进程，则由ICMP发送”端口不可达“差错报文给发送方。
...
显然，这**违反了分层原则**，看了网络层才有的IP，之所以要这么做时因为IP很重要，IP错了根本就传不到目标主机。伪首部不会真的发送，它单纯用来计算校验和。
...
3. 如果数据部分不是16bit的倍数需要补齐（但这部分字节不发送）
...
TCP 首部比UDP长，前20B是固定的，后面有4N字节是可扩展的

![](images/ch4/TCP.png)

TCP的首部的字段非常多，包括：

1. 源端口和目的端口，各占2B，和UDP一样
2. 序号(Sequence Number)，占4B,范围为0~2^32-1，TCP传输的字节流中的每个字节都要按顺序编号，序号字段值是指本报文段所发送数据的第一个字节序号
3. 确认号(Acknowledge Number)：占4B，**是期望收到对方下一个报文的序号**，和链路层讲的确认号不一样
...
8. 推送位（PSH）：PSH=1表示应该尽快把报文段交付给应用程序，不要等待
...
> 这个就是著名的三次握手，四次挥手，相信大部分人可能从一开始学计算机就知道有这个东西了，不过它到底是怎么一回事，且看下文分解
...
TCP的连接端口是套接字(Socket)，每一条TCP唯一地被通信的两个端点确定，还需要注意，同一个IP地址可能有多个不同的TCP连接，而同一个端口号也可以出现在多个TCP连接中。TCP连接采用Client/Server模式，这个会影响到后面HTTP的模式。
...
确保四次挥手的最后一次确认能被服务器接收：服务器如果发现MSL内都没收到ACK，那么可以判定为丢包，会重发FIN=1的报文，还未关闭连接的客户端收到后会重发ACK，一来一回时间就是2MSL
...
建立连接有一个问题是双方几乎同时发起连接会怎么样？答案是最终只会有一个连接。过程大致是双方发起的三次握手会交错进行，但是最终肯定有一方会先完成三次握手建立连接，由于TCP连接是全双工的，并且端到端之间的连接唯一，所以后”完成“实际不可能完成）的三次握手将失败。
...
1. 超时：每发送完报文段就设置一个计时器，超时重传。由于IP数据报所选择的路由差距变化很大，所以传输层往返时延的方差也很大。为此，TCP采用了一种自适应算法，记录一个报文段的发出时间，以及收到相应的确认时间，这两个时间之差称为报文段的往返时间（RTT），TCP维护了一个加权平均往返时间（Round-Trip-Time，RTTS）,TCP设置的超时重传时间（Retransmission Time-Out， RTO）应略大于RTTS，但也不能大太多，否则当报文段丢失时，TCP不能很快重传，导致数据传输时延大。
2. 冗余确认：在发生缺失后，每收到一个报文段就返回对已经收到的顺序报文段的ACK，TCP规定，当收到对同一个报文段的三个冗余ACK时，就可以认为跟在这个被确认报文段后的报文段已经丢失，可以立刻执行重传，这种技术通常被称作**快速重传**。

### 流量控制

> 接收方比发送方更有资格控制流量，发送方只管发就可以了，接收方要考虑的可就多了。。。

TCP 利用滑动窗口机制来实现流量控制，滑动窗口在链路层已经讲过了，但TCP的滑动窗口有点不同，它的窗口大小可以动态调整（链路层不行），使用的是报文头部的”窗口“字段，发送方的发送窗口不能超过接收方给出的接收窗口值。需要这么做的核心原因是接收方的接收缓存大小有限，并且接收方传输层可以选择传递数据给相应进程的时机。
...
拥塞控制和流量控制的区别是流量控制主要是针对接收方，而拥塞控制是针对网络中的路由器和链路，对于通信端点来说，拥塞表现为传输时延增加，但它们都是通过限制发送方发送速率来达成控制效果。除了流量控制中的接收窗口（rwnd），接收方还要维持拥塞窗口（cwnd），显然发送窗口应该取二者较小值。

拥塞控制的核心思想是在网络空闲时增大传输窗口，在网络拥塞时逐渐传输窗口，具体表现为慢开始、拥塞避免、快重传和快恢复

![](images/ch4/congestion_control.png)

慢开始指的并不是一开始的传输速度增长速率慢，反而一开始的增长的速率很快（指数级），慢开始指的是一开始的传输速度慢，因为如果不确定网络拥塞情况就使用很高的传输速度很容易导致拥塞。如上图所示，cwnd从1开始增长，每收到一个确认报文段就增加1，为了防止增长过快，还需要设置一个阈值（ssthresh），当cwnd增长到阈值时，改用拥塞避免算法。

拥塞避免算法的思路是让cwnd缓慢增大，每经过一个RTT就将cwnd加1，增长速率变为线性（加法增大）
