...
- 强制(mandatory)：访问会根据持有和请求的锁的状态被禁止
- 建议(advisory)：进程可以根据锁的状态决定该怎么做

#### 文件类型

!!!tip 关于扩展名（后缀）
    扩展名在Windows上很常见，但它只能起到提示应该拿什么软件处理这个文件的作用，扩展名也可以被看做文件名的一部分，所以之后的讨论都忽视扩展名
...
目录可以被看做用于翻译文件名到文件控制块(File Control Block, FCB)的符号表，这一系列的节点包含了所有文件的信息，目录和文件都在磁盘上。

典型的文件系统组织如下：

- 一个目录组织一系列文件，合在一起被称作一个分区(partition)
- 一个磁盘可能对应多个分区，也可能多个磁盘对应一个分区
- 还有一个概念是卷(Volumn)，一个磁盘上可以分出很多卷

![](images/ch8/file_organization.png)
...
![](images/ch8/graph.png)

- 解决对于文件共享的要求，支持共享子目录和文件
- 支持两个不同名字（别名）
- 如果目录被删除可能会导致悬空指针
  - 解决方案：维护文件的引用数，以便删除所有指针，但很大的可变大小引用数表是个问题
- 新的目录类型
  - 链接（link）：已经存在的文件的另一个名字（指针）
  - 解析链接：跟随指针去找到文件

无环图目录的一个大问题是如何保证无环，如果允许环，会导致：
...
- 软链接有自己的inode(FCB)，只包含路径；硬链接包含原始文件和指向同一个innode
- 软链接能够扩展文件系统，因为它们只是到其它文件的路径；硬链接不能创建到目录上，以免出现环

### 挂载

挂载（Mounting） 是指将一个存储设备（如硬盘分区、U盘、光盘等）的内容连接到文件系统中某个特定目录的过程。简单来说，硬件设备本身只是“一块砖”，挂载就是给它安一个“入口”，让你能通过文件夹看到里面的数据。

![](images/ch8/mounting.png)

挂载并不会影响到原来目录中的内容，毕竟挂载的是其它存储设备，原本的存储设备本身是不受影响的。
...
1. 应用程序
2. 逻辑文件系统：控制文件的元信息，保护和安全
3. 文件组织模块：将逻辑块翻译为物理地址
4. 基础文件系统：读写物理块的指令，I/O调度，缓冲
5. I/O控制：将读写指令翻译成更底层的磁盘读写指令

![](images/ch8/layer.png)

### 数据结构

磁盘上的数据结构：

- Boot control block(per volumn)
...
![](images/ch8/open_read.png)

### VFS

虚拟文件系统(Virtual File System)提供了一种面向对象的方式实现文件系统，也就是把具体文件系统再封装一层，以屏蔽不同文件系统的差异。具体而言，VFS允许同一个系统调用接口被用于不同类型的文件系统。
...
4. file object：和进程相关打开的文件相关，只要文件文件是打开的就一直存在
...
- 支持随机访问
- 浪费空间，因为随着不断地创建和删除会产生很多碎片
- 不支持文件增长

按照块分配可能会导致支持的最大容量不足（如今的磁盘不少是TB级的），所以不少新的系统调整为了按照extent（多个连续的块）分配

#### 链接分配

每个文件是磁盘块构成的链表，分配的块可以散布在磁盘中，比如一个块开头1 B是指向后一个块的指针：

- 简单，只需要起始地址
- 能够管理空闲空间，不会造成空间浪费
- 不支持随机访问

微软在此基础上想到可以把链表的信息集中在一起，然后缓存到内存中，从而提高访问随机地址的速度，这个想法被称作文件分配表（file allocation table），在lab6中会用到FAT32。不过这样做也带来一个问题，要是FAT损坏了，那文件信息不就全部丢失了吗？所以FAT实际会存储多份。
