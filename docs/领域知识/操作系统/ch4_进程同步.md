...
- **为什么？**：并发访问共享的数据可能导致数据不一致
- **解决方案**：维持数据一致性要求有机制(mechanism)能够保证合作进程按顺序执行。
...
这种问题被称作竞态条件(Race Condition)，它描述的是一处内存地址被并发访问，且至少有一个访问是写的情况。

!!!question "非抢占的内核是否受竞态条件的影响？"
    是，因为还是有并发的可能，比如发生了硬件中断，trap到内核线程

### 临界区问题

临界资源指的是在多进程（现成）环境中被多个进程（现成）共享的资源。这些资源需要通过适当的同步机制来保护，以确保同一时刻只有一个进程（线程）可以访问这些资源。为了设计一种协议来保证合作进程之间不会相互干扰，我们将进程抽象为如下结构：
...
4. 上课还讲了Yield（让权等待），即可以主动交出访问临界区的权限
...
- 谦让对方进入
  - 若双方互相确定对方都想进入时，`turn`只能等于一个值，只有一方能进入
  - 若一方不想进入，则对方可进入
...
但Peterson算法对于现代计算机体系结构不保证生效，因为存在乱序执行，实际执行顺序和看到的可能不一致。内存栅栏(memory barrier)可以避免乱序执行，栅栏前的操作必须全部完成并提交到内存后，才能执行栅栏后的操作
...
现代操作系统提供特殊的原子硬件指令，比如这里的TestAndSet，所谓的原子指令指的是同时只能有一个线程执行(由硬件保证)，不被打断。
...
!!!tip "和先前内容的关系"
    先前的解决方案都是站在最底层描述，相对来说较为复杂，一般来说在不涉及系统级开发时，programers 都喜欢用封装好的实现，比如这里的互斥锁。
...
必须保证同时只有一个进程能执行同一个信号量的`wait`和`signal`，所以信号量本身也是临界区问题，可以使用忙等实现，但是如果等待时间较长，忙等的开销也不小，忙等的示例如下：
...
N buffers，每个可以容纳一个元素，只有一个Producer和一个Consumer我们的需求是：
...
读者优先是指允许多位读者一起读，有且仅有一位写者能在没有读者时开始写，我们的需求是：
...
读者可以一起读，不过读者只有在没有写者处于等待中的情况下才能开始读，只能有一位写者在写
...
semaphore block_readers = 1; // 阻塞新读者
...
哲学家面前有一碗饭，左右手各一只，需要同时拿起一双筷子才能吃饭。问题本身并不复杂，但是下面这种实现可能导致死锁，解决起来不难，比如只允许四个人同时吃饭，其中有一个反序拿筷等
...
管程是站在封装角度考量，因为到处用`wait`和`signal`还是有点麻烦，干脆直接将同步逻辑封装成一个类（代码块），保证同一时间只能有一个进程在管程中执行。管程中会使用条件变量，提供`wait`和`signal`两个方法，但效果和普通的信号量不同。